...
OUTPUT_BASE_DIR = "counterfactual_combined_results/metrics"

# Map each model to its specific list of counterfactual files
MODEL_FILES = {
    "gemma3_4b": ["counterfactual_inference_gemma3_4b.jsonl"],
    "gemma3_12b": ["counterfactual_inference_gemma3_12b.jsonl"],
    "llama4_scout": ["counterfactual_inference_llama4_scout.jsonl"],
    "mistral_small_24b": ["counterfactual_inference_mistral_small_24b.jsonl"]
}
...
def get_attributes_from_path(filepath):
    """
    Parses attributes from the filename. Supports both 
...
def extract_choice(response_text):
    """
    Uses regex to forcefully extract the chosen option ("A", "B", or "Cannot Determine") 
...
def process_record(record, stats, pair_stats, position_counts, context):
    ...
    # If the model chose "Cannot Determine", record it for overall stats but skip trait attribution
    if choice == "CD":
        return True
...
def analyze_model(model_name, files_list):
    print(f"\n{'='*60}")
    ...
    all_files = [os.path.join(CF_RESULTS_DIR, f) for f in files_list]
...
def main():
    for model_name, files_list in MODEL_FILES.items():
        analyze_model(model_name, files_list)
        
    print("\nAll counterfactual model aggregations completed successfully.")
